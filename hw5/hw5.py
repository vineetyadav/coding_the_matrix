...
def morph(S, B):
    ...
    for b in B:  
        Inject.append(b)
        C = copy(S)
        for s_i in range(len(C)):
            T=Inject+C[:s_i]+C[s_i+1:]
            M=coldict2mat(T)
            u=solve(M,C[s_i])
            residue=C[s_i]-M*u
        
            if residue*residue<10**-14: 
                ret.append((b,C[s_i]))
                del(C[s_i])
                break               
...
def direct_sum_decompose(U_basis, V_basis, w):
    ...
    for x in range(len(U_basis)):
        Wu[x]=W[x]
    for x in range(len(V_basis)):
        Wv[x]=W[len(U_basis)+x]
    u=U*Wu
    v=V*Wv
    return (u,v)
...
def is_invertible(M): 
    ...
    '''
    S = mat2coldict(M)
    L = [v for (k,v) in S.items()]
    if (len(M.D[0]) == len(M.D[1])) and (rank(L) == len(L)):
      return True
    return False
...
def identity(D, x):
    ...
    return Mat((D,D), {(d,d):x for d in D})
...
def find_matrix_inverse(A):
    ...
    R= mat2coldict(I)
    M=[solve(A,R[i]) for i in range(len(R))]
...
def find_triangular_matrix_inverse(A): 
    ...
    I = identity(A.D[0], 1)
    R= mat2coldict(I)
    #b = list(mat2rowdict(I).values())  
    #M = list(mat2rowdict(A).values()) 
    #return triangular_solve_n(M,b)
    M=[triangular_solve(list(mat2rowdict(A).values()),range(len(R)),R[i]) for i in range(len(R))]
    return coldict2mat(M)
