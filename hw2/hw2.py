...
def vec_select(veclist, k): 
    ...
    True
    '''
    return [each for each in veclist if each.f.get(k,0)==0]
    #pass
...
def vec_select_sum(veclist, k, D): 
    ...
    if not veclist:
        ...
        return sum(vec_select(veclist,k))
...
def scale_vecs(vecdict):
    ...
    rlist = []
    for each in vecdict:
        for each_v in vecdict[each].f:
            vecdict[each].f[each_v] = (1.0/each)*vecdict[each].f[each_v]
        rlist.append(vecdict[each])
